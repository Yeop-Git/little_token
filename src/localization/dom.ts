...
const ROWS: Row[] = [
  ...
  ['토큰이 알려 줄게!','Token will show you!','トークンが教えるよ！','Токен всё объяснит!','让词元告诉你！','讓詞元告訴你！'], ['멋진 문장을 쓰는 법','How to write a great sentence','すてきな文の書き方','Как написать отличную фразу','写出精彩句子的方法','寫出精彩句子的方法'],
...
export function translateText(value: string, locale: ForeignLocale): string {
  ...
  if (inspirationShortage) {
    ...
    const message = locale === 'ja' ? `ひらめきが${amount}足りず、この報酬を記録できなかった。` : locale === 'ru' ? `Не хватает вдохновения: ${amount}. Награда не записана.` : locale === 'zh-Hans' ? `灵感还差${amount}，无法记录该奖励。` : locale === 'zh-Hant' ? `靈感還差${amount}，無法記錄該獎勵。` : `Not enough Inspiration (${amount} short). The reward was skipped.`
