...
const ROWS: Row[] = [
  ...
  ['영감','Inspiration','ひらめき','Вдохновение','灵感','靈感'], ['클리어 보상','Clear reward','クリア報酬','Награда за победу','通关奖励','通關獎勵'], ['보상','Reward','報酬','Награда','奖励','獎勵'], ['주인공 상태','Hero status','主人公の状態','Состояние героя','主角状态','主角狀態'],
  ...
  ['토큰의 메모','Token’s note','トークンのメモ','Заметка Токена','词元的笔记','詞元的筆記'], ['문장 규칙 다시보기','Sentence rules recap','文のルールを見直す','Повтор правил фразы','回顾句子规则','回顧句子規則'],
  ...
  ['보유 영감','Inspiration','所持ひらめき','Вдохновение','持有灵感','持有靈感'], ['이번 클리어','This clear','今回のクリア','За эту победу','本次通关','本次通關'], ['기록하기 →','Record →','記録する →','Записать →','记录 →','記錄 →'], ['다른 발상 떠올리기','Think of something else','別の発想を思いつく','Придумать другое','换个想法','換個想法'], ['그냥 넘어가기','Continue without recording','何も記録せず進む','Продолжить без записи','直接跳过','直接跳過'], ['영감이 부족해 다른 발상을 떠올릴 수 없다.','Not enough Inspiration to think of something else.','ひらめきが足りず、別の発想を思いつけない。','Не хватает вдохновения для новой идеи.','灵感不足，无法换个想法。','靈感不足，無法換個想法。'], ['자세히보기','Details','詳細','Подробнее','详情','詳情'], ['새 단어','New word','新しい言葉','Новое слово','新词语','新詞語'], ['아이템','Item','アイテム','Предмет','道具','道具'],
  ['내 단어장에 기록!','Added to my wordbook!','自分の単語帳に記録！','Записано в мой словарь!','已记入我的词册！','已記入我的詞冊！'], ['강화 완료!','Upgrade complete!','強化完了！','Усиление завершено!','强化完成！','強化完成！'], ['내 소품으로 결정!','This prop is mine!','自分の小道具に決定！','Этот предмет — мой!','决定作为我的道具！','決定作為我的道具！'],
  ...
  ['획득 예정 영감 — 오래 끌면 줄고, 한 턴에 쓸어담으면 늘어난다','Expected Inspiration — falls in long battles and rises when you sweep a turn','獲得予定のひらめき — 長期戦で減り、1ターンで一掃すると増える','Ожидаемое вдохновение уменьшается в долгом бою и растёт за зачистку за ход','预计获得的灵感——战斗拖久会减少，一回合清场则增加','預計獲得的靈感——戰鬥拖久會減少，一回合清場則增加'],
  ...
  ['쓰러진 장로거미','Fallen Elder Spider','倒れた長老蜘蛛','Павший Старый паук','倒下的长老蜘蛛','倒下的長老蜘蛛'], ['고개를 떨군 토큰','Downcast Token','うなだれたトークン','Поникший Токен','垂头丧气的词元','垂頭喪氣的詞元'], ['나팔을 부는 토큰','Trumpeting Token','ラッパを吹くトークン','Токен с трубой','吹响号角的词元','吹響號角的詞元'],
  ...
  ['토큰','Token','トークン','Токен','词元','詞元'], ['프롬','Prompt','プロンプト','Промпт','提示词','提示詞'],
  ...
  ['장로거미가 고쳐 쓰던 문장은 멈췄다. 프롬은 찢어진 페이지를 한 장씩 펴서, 자신의 글씨로 결말을 다시 잇는다.','The Elder Spider’s rewriting stopped. Prompt smooths each torn page and reconnects the ending in his own hand.','長老グモが書き換えていた文は止まった。プロンプトは破れたページを一枚ずつ広げ、自分の字で結末をつなぎ直す。','Старый паук больше не переписывает фразы. Промпт разглаживает порванные страницы и собственноручно восстанавливает финал.','长老蜘蛛篡改的句子停了下来。提示词逐页铺平破损的纸张，用自己的笔迹重新续写结局。','長老蜘蛛篡改的句子停了下來。提示詞逐頁鋪平破損的紙張，用自己的筆跡重新續寫結局。'],
  ['토큰이 되찾은 문장 위를 환하게 맴돈다. 끝이라고 적은 자리 너머에도, 아직 쓰지 않은 페이지가 바람에 팔랑인다.','Token circles brightly above the recovered sentence. Beyond the word “End,” unwritten pages flutter in the wind.','トークンは取り戻した文の上を明るく舞う。「終わり」と書いた先でも、まだ白いページが風に揺れている。','Токен сияет над возвращённой фразой. За словом «Конец» на ветру шелестят ещё не написанные страницы.','词元在夺回的句子上方明亮地盘旋。即使越过写着“完”的地方，尚未落笔的书页仍在风中翻飞。','詞元在奪回的句子上方明亮地盤旋。即使越過寫著「完」的地方，尚未落筆的書頁仍在風中翻飛。'],
...
/** DOM 사전 자체의 빈 칸·한국어 잔존·충돌 키를 전수 검사한다. */
export function domLocalizationErrors(): string[] {
  const errors: string[] = []
  const seen = new Map<string, Row>()
  for (const [rowIndex, row] of ROWS.entries()) {
    if (!row[0].trim()) errors.push(`DOM row ${rowIndex}: empty Korean source`)
    const previous = seen.get(row[0])
    if (previous && previous.slice(1).some((value, index) => value !== row[index + 1])) {
      errors.push(`DOM row ${rowIndex}: conflicting duplicate source ${row[0]}`)
    }
    seen.set(row[0], row)
    for (const [locale, column] of Object.entries(INDEX) as [ForeignLocale, number][]) {
      const text = row[column]
      if (!text?.trim()) errors.push(`${locale}: empty DOM translation for ${row[0]}`)
      if (/[가-힣]/.test(text)) errors.push(`${locale}: Korean remains in DOM translation for ${row[0]}`)
    }
  }
  return errors
}
...
export function translateText(value: string, locale: ForeignLocale): string {
  ...
  const inspirationShortage = core.match(/^영감이 (\d+) 부족해 이 보상을 기록하지 못했다\.$/)
  if (inspirationShortage) {
    const amount = inspirationShortage[1]
    const message = locale === 'ja' ? `ひらめきが${amount}足りず、この報酬を記録できなかった。` : locale === 'ru' ? `Не хватает вдохновения: ${amount}. Награда не записана.` : locale === 'zh-Hans' ? `灵感还差${amount}，无法记录该奖励。` : locale === 'zh-Hant' ? `靈感還差${amount}，無法記錄該獎勵。` : `Not enough Inspiration (${amount} short). Choose another reward.`
