...
const UI: Record<Exclude<LocaleCode, 'ko'>, UiDictionary> = {
  en: {
    ...
    hudStageLabel: 'Stage', hudStoryCycle: 'First story', hudEndlessCycle: 'Endless cycle {cycle}', hudEndlessStage: 'ENDLESS · STAGE {day}', hudEndlessCycleProgress: 'Cycle {cycle} · {floor}/{total}', hudEndlessStageAria: 'Endless cycle {cycle}. Cycle stage {floor} of {total}. Overall stage {day}. {next}', hudBoss: 'Boss', hudBossNow: '{boss} battle', hudNextBoss: '{count} stages to {boss}', hudStageAria: '{cycle}. Stage {floor} of {total}. {next}',
    rewardFactRarity: 'Rarity', rewardFactRole: 'Role', rewardFactAcquire: 'Gain', rewardFactCost: 'Cost',
    rewardSlotSubject: 'Subject', rewardSlotModifier: 'Modifier', rewardSlotVerb: 'Verb', rewardSlotObject: 'Object', rewardSlotEnding: 'Ending', rewardSlotSentence: 'Sentence', rewardSlotOrder: '{order}. {slot}', rewardTypeItem: 'Item', rewardItemPassive: 'Passive', rewardItemBaseStats: 'Innate base stats',
    ...
    rewardDetailHint: 'Hover over a card to see its effects, odds, and affected stats here.',
  ...
  ja: {
    ...
    hudHelpTip: '戦闘ヘルプ\n文を組み立てる基本ルールを見直す。',
    hudStageLabel: 'ステージ', hudStoryCycle: '最初の物語', hudEndlessCycle: 'エンドレス {cycle}周目', hudEndlessStage: 'エンドレス · {day}ステージ', hudEndlessCycleProgress: '{cycle}周目 · {floor}/{total}', hudEndlessStageAria: 'エンドレス{cycle}周目。周回ステージ{floor}/{total}。通算ステージ{day}。{next}', hudBoss: 'ボス', hudBossNow: '{boss}戦', hudNextBoss: '{boss}まであと{count}ステージ', hudStageAria: '{cycle}。ステージ{floor}/{total}。{next}',
    rewardFactRarity: 'レア度', rewardFactRole: '役割', rewardFactAcquire: '獲得', rewardFactCost: '費用',
    rewardSlotSubject: '主語', rewardSlotModifier: '修飾語', rewardSlotVerb: '動詞', rewardSlotObject: '目的語', rewardSlotEnding: '語尾', rewardSlotSentence: '文', rewardSlotOrder: '{order}番 {slot}', rewardTypeItem: 'アイテム', rewardItemPassive: 'パッシブ', rewardItemBaseStats: '固有基本能力',
    ...
    rewardDetailHint: 'カードにカーソルを合わせると、効果・確率・影響する能力値がここに表示されます。',
  ...
  ru: {
    ...
    hudStageLabel: 'Этап', hudStoryCycle: 'Первая история', hudEndlessCycle: 'Бесконечный цикл {cycle}', hudEndlessStage: 'БЕСКОНЕЧНОСТЬ · ЭТАП {day}', hudEndlessCycleProgress: 'Цикл {cycle} · {floor}/{total}', hudEndlessStageAria: 'Бесконечный цикл {cycle}. Этап цикла {floor} из {total}. Общий этап {day}. {next}', hudBoss: 'Босс', hudBossNow: 'Бой: {boss}', hudNextBoss: 'До {boss}: {count} этап.', hudStageAria: '{cycle}. Этап {floor} из {total}. {next}',
    rewardFactRarity: 'Редкость', rewardFactRole: 'Роль', rewardFactAcquire: 'Получение', rewardFactCost: 'Цена',
    rewardSlotSubject: 'Подлежащее', rewardSlotModifier: 'Модификатор', rewardSlotVerb: 'Глагол', rewardSlotObject: 'Дополнение', rewardSlotEnding: 'Окончание', rewardSlotSentence: 'Фраза', rewardSlotOrder: '{order}. {slot}', rewardTypeItem: 'Предмет', rewardItemPassive: 'Пассивный эффект', rewardItemBaseStats: 'Врождённые параметры',
    ...
    rewardDetailHint: 'Наведите курсор на карту, чтобы увидеть здесь эффекты, шансы и связанные параметры.',
  ...
  'zh-Hans': {
    ...
    hudHomeTip: '主页\n将本场战斗重置到开始前并返回标题画面。',
    hudHelpTip: '战斗帮助\n回顾组装句子的核心规则。',
    hudStageLabel: '关卡', hudStoryCycle: '第一篇故事', hudEndlessCycle: '无尽第{cycle}轮', hudEndlessStage: '无尽 · 第{day}关', hudEndlessCycleProgress: '第{cycle}轮 · {floor}/{total}', hudEndlessStageAria: '无尽第{cycle}轮。本轮第{floor}/{total}关。累计第{day}关。{next}', hudBoss: '首领', hudBossNow: '{boss}首领战', hudNextBoss: '距离{boss}还有{count}关', hudStageAria: '{cycle}。第{floor}/{total}关。{next}',
    rewardFactRarity: '稀有度', rewardFactRole: '作用', rewardFactAcquire: '获得', rewardFactCost: '费用',
    rewardSlotSubject: '主语', rewardSlotModifier: '修饰语', rewardSlotVerb: '动词', rewardSlotObject: '宾语', rewardSlotEnding: '句尾', rewardSlotSentence: '句子', rewardSlotOrder: '第{order}位 {slot}', rewardTypeItem: '道具', rewardItemPassive: '被动能力', rewardItemBaseStats: '固有基础属性',
    ...
    rewardDetailHint: '将鼠标悬停在卡牌上，即可在此查看效果、概率和影响属性。',
  ...
  'zh-Hant': {
    ...
    hudHomeTip: '主頁\n將本場戰鬥重置到開始前並返回標題畫面。',
    hudHelpTip: '戰鬥說明\n回顧組裝句子的核心規則。',
    hudStageLabel: '關卡', hudStoryCycle: '第一篇故事', hudEndlessCycle: '無盡第{cycle}輪', hudEndlessStage: '無盡 · 第{day}關', hudEndlessCycleProgress: '第{cycle}輪 · {floor}/{total}', hudEndlessStageAria: '無盡第{cycle}輪。本輪第{floor}/{total}關。累計第{day}關。{next}', hudBoss: '首領', hudBossNow: '{boss}首領戰', hudNextBoss: '距離{boss}還有{count}關', hudStageAria: '{cycle}。第{floor}/{total}關。{next}',
    rewardFactRarity: '稀有度', rewardFactRole: '作用', rewardFactAcquire: '獲得', rewardFactCost: '費用',
    rewardSlotSubject: '主語', rewardSlotModifier: '修飾語', rewardSlotVerb: '動詞', rewardSlotObject: '受詞', rewardSlotEnding: '句尾', rewardSlotSentence: '句子', rewardSlotOrder: '第{order}位 {slot}', rewardTypeItem: '道具', rewardItemPassive: '被動能力', rewardItemBaseStats: '固有基礎屬性',
    ...
    rewardDetailHint: '將滑鼠懸停在卡牌上，即可在此查看效果、機率和影響屬性。',
