...
모든 아이템의 고정 기본 스탯은 0이고, 플레이어가 감탄 문장을 조립하면 등급 예산 안에서
선택한 단어의 보정이 붙는다. 코어 루프(문장 조립)의 축소판이라 규칙이 일관된다.
...
모든 등급은 CSV 기본 스탯이 0이고 같은 감탄사 3칸을 골라 선택 스탯을 받는다.
노멀·희귀는 선택 스탯만, 영웅·전설은 여기에 **문장을 읽는 규칙**을 더한다.
