...
- 전투의 각 문장은 `SENTENCE_INK = 10` 잉크로 시작한다. 잉크는 다음 문장을
  조립할 때 다시 10으로 채워지며 스테이지 사이에 저장하지 않는다.
- 단어 비용은 `core/ink.ts`의 `wordInkCost()`가 현재 희귀도와 실제 성능에서
  계산한다. 희귀도가 비용의 바닥을 정하고, 높은 계수·배율·대성공 확률·광역·
  다중타격·관통 같은 강한 규칙이 비용을 추가한다.
- 비용 합계가 10을 넘어도 선택을 차단하지 않는다. 문장을 확정할 때
  `max(0, 총비용 - 10)`만큼 체력을 한 번 직접 지불하며 방어막은 소모하지 않는다.
...
- 기본 보정은 스탯 +1씩, 체력만 +2다. 모든 선택은 최소 1점이고 등급 보너스는
  노멀 0점·희귀 1점·영웅 2점·전설 3점이다. 최종 총예산은 3·4·5·6점이다
  (체력 2 = 다른 스탯 1, 행운 감탄 제외). 보너스가 나타나는 감탄 단계만 화면 입장 시
...
| 올림프의 황금 당근 | `punct` | 문장 끝에 문장부호 칸 | `slotOrderFor()` + `PUNCT_WORDS` |
| 두 겹의 문장과자 | `twinVerb` | 동사 칸이 둘 | `slotOrderFor()` + `words.verb2` |
| 미녀의 은빛 거울 | `twinSubj` | 주어 칸이 둘 | `slotOrderFor()` + `words.subj2` |
| 백설공주의 유리구두 | `heavyShoe` | 피해 깡수치에 문장당 +7 | `CompileMods.verbFlat` |
| 빨간망토의 불씨 | `matchFire` | 배율 칸마다 보너스 +0.10 | `CompileMods.bonusEach` |
| 성냥팔이의 붉은 망토 | `luckCloak` | 모든 동사가 운만큼 깡수치를 더 받음 | `CompileMods.verbLuck` |
| 아기돼지의 숯불화로 | `bbq` | 이번 전투 처치 1마리당 배율 +5% | `CompileMods.stageMult` |
| 피노키오의 물음표 | `doubt` | 문장을 완성하면 끝에 `…근데?`가 붙어 ×1.00~1.30 **한 번** | `compile()` · `doubtMults()` |
| 잭의 하늘나물 | `beanstalk` | 모든 칸에 무럭무럭 카드가 섞인다(고르면 최대체력 +2) | `makeEarlyTables()` |
