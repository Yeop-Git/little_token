...
4. 일반 방어막은 먼저 피해를 받으며, `관통`은 방어막을 무시하고 소모시키지 않는다. 매직실드는 한 타격을 완전히 막으므로 `연타`로 벗길 수 있다.
...
노멀·희귀 아이템은 **스탯**을 준다. 전설 아이템은 기본 스탯이 0이고 **문장을 읽는 규칙**을 바꾼다.
...
아이템 보상은 매번 **일반 스탯 아이템 2장 + 전설 규칙 아이템 1장**의 3택으로
보여 준다. 전설 칸은 아직 얻지 않은 규칙 아이템을 우선하고, 전설 11종을 모두
획득한 뒤에는 전체 전설 풀에서 다시 뽑는다.
